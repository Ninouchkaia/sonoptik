...
to go
  ...
  file-open "simu_output_gendrift_1.txt"
  output_coordinates
  tick
  file-close
end
...
to file_write [my_breed]
  let x1 xcor - min-pxcor
  let x round (x1 * (800 / (max-pxcor - min-pxcor)))
  let y1 ycor + min-pycor
  let y round (y1 * (800 / (min-pycor - max-pycor)))
